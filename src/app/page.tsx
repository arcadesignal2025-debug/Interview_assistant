...
export default function Home() {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      if (data.done) {
        ...
        if (data.skillChart) setSkillChart(data.skillChart);
        setView('feedback');
      }
    } catch (err) {
  ...
  const {
    ...
    onViolationTerminate: handleViolationTerminate,
  });
    ...
    <div className="min-h-screen bg-dark-bg selection:bg-violet-deep selection:text-white">
      ...
      {view === 'interview' && selectedCandidate && (
        <InterviewInterface
          ...
          onEndEarly={() => setView('feedback')}
          onBackToSelection={() => setView('selector')}
          proctorActive={true}
        />
      )}
      ...
      <ProctorWarningModal
        ...
        onCancelBackLeave={() => setBackModalOpen(false)}
      />
    </div>
  );
}
